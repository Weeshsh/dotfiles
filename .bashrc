...
# Define how many directories to show (1, 2, or 3)
DIR_COUNT=3

if [ "$color_prompt" = yes ]; then
    if [ "$DIR_COUNT" -eq 1 ]; then
        PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]${PWD##*/}\[\033[00m\]\$ '
    elif [ "$DIR_COUNT" -eq 2 ]; then
        PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]$(basename "$(dirname "$PWD")")/$(basename "$PWD")\[\033[00m\]\$ '
    elif [ "$DIR_COUNT" -eq 3 ]; then
        PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]$(basename "$(dirname "$(dirname "$PWD")")")/$(basename "$(dirname "$PWD")")/$(basename "$PWD")\[\033[00m\]\$ '
    else
        PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]${PWD##*/}\[\033[00m\]\$ '
    fi
else
    if [ "$DIR_COUNT" -eq 1 ]; then
        ...
        PS1='${debian_chroot:+($debian_chroot)}\u@\h:"$(basename "$(dirname "$PWD")")/$(basename "$PWD")"\$ '
    elif [ "$DIR_COUNT" -eq 3 ]; then
        PS1='${debian_chroot:+($debian_chroot)}\u@\h:"$(basename "$(dirname "$(dirname "$PWD")")")/$(basename "$(dirname "$PWD")")/$(basename "$PWD")"\$ '
    else
        PS1='${debian_chroot:+($debian_chroot)}\u@\h:"${PWD##*/}"\$ '
    fi
fi
...
# oh my posh
# eval "$(oh-my-posh init bash --config 'https://raw.githubusercontent.com/JanDeDobbeleer/oh-my-posh/main/themes/darkblood.omp.json')"
